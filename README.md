...
- ⚠️ Многоязычный OCR (`rus+eng`): liteparse 2.0.x для склеенного языка ищет
  один файл `rus+eng.traineddata` и при его отсутствии лезет качать с github
  (→ 404, в offline тем более), хотя сам tesseract корректно разбивает строку
  по `+`. Поэтому `Dockerfile` создаёт симлинк-маркер
  `rus+eng.traineddata` → `eng.traineddata` (и обратный `eng+rus`), чтобы
  проверка существования прошла и скачивание не запускалось. Для нового
  склеенного языка добавьте аналогичный симлинк рядом с `COPY tessdata`.
